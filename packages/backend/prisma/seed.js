...
async function main() {
    ...
    for (const ec of equipCategories) {
        createdCategories.push(await prisma.equipmentCategory.create({ data: ec }).catch(() => prisma.equipmentCategory.findFirst({ where: { categoryName: ec.categoryName } }).then(r => r)));
    }
