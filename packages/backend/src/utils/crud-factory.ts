...
export function createCrudRouter(config: CrudConfig): Router {
  ...
  router.get('/', authenticate, paginate(config.defaultSort || 'id'), async (req: Request, res: Response, next: NextFunction) => {
    try {
  ...
  const writeMw = config.allowedRoles?.length
    ? [authenticate, requireRole(...config.allowedRoles)]
    : [authenticate];

  // POST / - Create
  router.post('/', ...writeMw, validate(config.createSchema), async (req: Request, res: Response, next: NextFunction) => {
    try {
  ...
  // PUT /:id - Update
  router.put('/:id', ...writeMw, validate(config.updateSchema), async (req: Request, res: Response, next: NextFunction) => {
    try {
