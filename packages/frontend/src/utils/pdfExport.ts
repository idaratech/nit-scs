  ...
  if (options.lineItems && options.lineItems.length > 0 && options.lineItemColumns) {
    ...
    autoTable(doc, {
      ...
      body: options.lineItems.map(item =>
        options.lineItemColumns!.map(col => String(item[col] ?? ''))
      ),
      theme: 'grid',
...
export function buildPdfOptions(
  ...
  const docNumber = String(
    record.mrrvNumber || record.mirvNumber || record.mrvNumber ||
    record.joNumber || record.rfimNumber || record.osdNumber ||
    record.gatePassNumber || record.transferNumber || record.shipmentNumber ||
    record.id || ''
  );
  const common = {
    docNumber,
    ...
    case 'mrrv':
      return {
        ...
        fields: [
          ...
          { label: 'Warehouse', value: String((record as Record<string, unknown>).warehouse || record.warehouseId || '') },
